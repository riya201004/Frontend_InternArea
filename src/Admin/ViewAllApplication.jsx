  ...
  const { t, i18n } = useTranslation();
    ...
    useEffect(()=>{
        ...
        try {
          ...
          setApplication(translatedApplications)
...
console.log(application)
  return (
    <div>
        <div className='hide'>
     <LanguageSwitcher />
    <h1 className='text-3xl font-semibold mt-3'>{t('application.totalApplications')}</h1>
...
<thead className='border-b font-medium'>
    <tr className='bg-gray-200'>
        <th scope='col' className='px-5 py-4'>{t('application.company')}</th>
        <th scope='col' className='px-5 py-4'>{t('application.category')}</th>
        <th scope='col' className='px-5 py-4'>{t('application.appliedOn')}</th>
        <th scope='col' className='px-5 py-4'>{t('application.appliedBy')}</th>
        <th scope='col' className='px-5 py-4'>{t('application.viewDetail')}</th>
        <th scope='col' className='px-5 py-4'>{t('application.applicationStatus')}</th>
...
<tbody>
    ...
    <div className='show'>
        <h1>{t('application.viewAllApplications')}</h1>
        {application.map((data)=>(
     ...
     <div class="flex flex-wrap -m-4">
       <div class="p-4 lg:w-1/2 md:w-full">
         <div class="flex border-2 rounded-lg border-gray-200 border-opacity-50 p-8 sm:flex-row flex-col">
           ...
           <div class="flex-grow">
             <h2 class="text-gray-900 text-lg title-font font-medium mb-3"> {t('application.companyName')} {data.company}</h2>
             <p class="leading-relaxed text-base"> {t('application.appliedBy')}  {data.user.name}</p>
             <p class="leading-relaxed text-base"> {t('application.appliedOn')}  {new Date(data?.createAt).toLocaleDateString()}</p>
             <p class="leading-relaxed text-base"> {t('application.applicationStatus')}  {data.status}</p>
             <Link to={`/detailApplication?a=${data._id}`} class="mt-3 text-indigo-500 inline-flex items-center">{t('intern.viewInDetail')}
